...
if(	isset($_POST['action']) &&
	...
	$generate_challan_date=date('m');
  	$challan_number_generate=return_next_id("id", "com_create_challan_mst", "1");
  	$challan_number_generate='OH-'.$hidden_short_name2.'-C-'.$generate_challan_date.'-'.$challan_number_generate;
...
if(isset($_GET['action']) && isset($_GET['update_id1']) && isset($_GET['update_quotation_id2'])){
	...
	if($action_csv=="csv_export")
	{
		ob_start();	
		$i=0;
		//  Full texts 	 Full texts 	id 	quotation_mst_id 	order_mst_id 	challan_number_generate 	challan_subject 	challan_date 	is_deleted 	status_active 	insert_date 	inserted_by 	update_date 	updated_by
...
$result_mst=mysql_query("select a.id,a.challan_subject,a.challan_number_generate,a.challan_date, b.id,b.to_name,
	b.to_designation,b.to_company,b.to_address,b.total_amount,b.vat,b.ait,b.total_amount_with_vat,b.total_amount_in_word 
	FROM com_create_challan_mst a,com_create_quotation_mst b 
		 ...
		 from com_create_quotation_dtls where mst_id= $update_quotation_id3 and is_deleted=0 and status_active=1 ");
				    ...
				    <div style="width:100%;">
							    ...
							    foreach ($array_mst as $row_mst)
							    {
							    	//$slNo++;
								?>	<div>	
										<div style="text-align:center;">Challan</div>
										<div style="text-align:right;"><?php echo $row_mst['challan_date']; ?></div>
										<div style="text-align:right;"><?php echo $row_mst['challan_number_generate']; ?></div>
								   	</div>	
								   	<div>	
								    	<div>To</div>
								    	...
								    	<div><?php echo $row_mst['challan_subject']; ?></div>
...
<?php
	...
	$filename='../../CSV_FILE/challan/'.$title.$name."_".$generateTime.".xls";
