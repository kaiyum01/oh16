...
if(	isset($_POST['action']) &&
	...
	$generate_bill_date=date('d-m-y');
  	...
  	$bill_number_generate='OH-'.$hidden_short_name2.'-Bill-'.$bill_number_generate.'-'.$generate_bill_date;
	...
	*/
	...
	{
	  	$query = mysql_query("insert into com_create_bill_mst(quotation_mst_id,order_mst_id,bill_number_generate,bill_subject,bill_date,insert_date,inserted_by) values('$hidden_quotation_id2','$hidden_order_id2','$bill_number_generate','$to_subject2','$only_today_date','$insert_and_update_date','$login_session_user_id')");
	  	/*
	  	$data_array_cus_lc="";
		//dtls
...
if(	isset($_POST['action']) &&
	...
	{
	//update query 
	  $query_update = mysql_query("update com_create_bill_mst SET bill_subject='$to_subject2',update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id2'");
	  if($query_update==1){
	  	echo $msg_update;
...
if(isset($_GET['action'])){
	...
	{
		$i=0;
		$result=mysql_query("select a.id as bill_id,a.bill_number_generate,a.bill_subject,a.bill_date,a.order_mst_id,b.id as quotaion_id,b.to_company,b.total_amount_with_vat from com_create_bill_mst a,com_create_quotation_mst b where a.quotation_mst_id=b.id order by a.id DESC");
		?>
